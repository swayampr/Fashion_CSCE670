import csv, gzip, itertools, json, os, random, torch, torchvision
import numpy as np 
...
# Dataset for polyvore with 5 categories (upper, bottom, shoe, bag, accessory), each outfit has at least 3 items, the missing items will use a mean image.
class CategoryDataset(Dataset):
    def __init__(self,
                 root_dir="/root/fashion_CSCE670/data/images/",
                 data_file='train_no_dup_with_category_3more_name.json',
                 data_dir="/root/fashion_CSCE670/data",
                 transform=None,
                 use_mean_img=True,
                 neg_samples=True):
    ...
    # It could return a positive outfits or negative outfits
...
# Need custom a collate_fn for LSTM DataLoader. Batch images will be transformed to a long sequence.
...
# Dataset will generate triplet to train conditional similarity network. Each element in dataset should be anchor image, positive image, negative image and condition.
